...
import { alternarMaquina, guardarMaquina, type DatosMaquina } from '@/datos/parque'
...
export function FormularioMaquina({
  ...
  /** Recarga lo que haya detrás. Guardar cambia el parque y su semáforo. */
  onCambio: () => void
}) {
  const [guardando, iniciar] = useOcupado()
  const [alternando, iniciarAlternar] = useOcupado()
  ...
  function alternar() {
    ...
    iniciarAlternar(async () => {
      const r = await alternarMaquina(maquina.id, !maquina.activa)
      if (r.ok) {
